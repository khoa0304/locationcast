...
public class LocationRepositoryTest {
	...
	@Test
	public void testInsertCoordinatesToDB(){
		
		DefaultIndexOperations indexOp = new DefaultIndexOperations(mongoOperation,Location.class.getSimpleName());
		indexOp.dropAllIndexes();
		
		mongoOperation.dropCollection(Location.class);
		mongoOperation.createCollection(Location.class);
		
		GeospatialIndex geoIndex = new GeospatialIndex(Location.AttributeNames.point.getName());
	    ...
	    List<Location> longLatArrayLits =  new ArrayList<Location>(longLatArray.length);
		int i = 1;
	...
	//		System.out.printf("Longitude: %f - Latitude: %f \n ", coordiate[0],coordiate[1]);			
		...
		query.addCriteria(Criteria.where(Location.AttributeNames.id.getName()).gt(0));
    ...
    public void testQueryForMaxDistanceInMile() {
		 ...
		 Criteria criteria = new Criteria("point").near(new Point(startingLongitude, startingLatitude)).maxDistance(getInMile(0.0001));
	     Query query = new Query(criteria);
	     query.with(new Sort(Sort.Direction.ASC,Location.AttributeNames.id.getName()));
		 List<Location> locationList = mongoOperation.find(query,Location.class);
	...
	static double startingLatitude = 37.319337; //37.999999  37.319337,-121.839370
	static double startingLongitude = -121.839370; //-121.800000 to -122.8000000
	static double DecrementFactor = -0.000001;
	static double IncrementFactor = 0.000001;
	
	static int numberOfCoordinate = 50000;
	...
	public static double[][] getLongitudeLatitudeDataset(double fraction) {
		...
		for(int i = 0 ; i < numberOfCoordinate; i ++){
			...
			startingLongitude  = Double.valueOf(form.format(startingLongitude + fraction));
			startingLatitude   = Double.valueOf(form.format(startingLatitude + fraction));
